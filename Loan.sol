...
/**
 * @title TribrOne Lending Contract
...
contract LendingData is ERC721Holder, ERC1155Holder, Ownable {
  ...
  enum TimeScale{ MINUTES, HOURS, DAYS, WEEKS, MONTHS }
  ...
  TimeScale public installmentTimeScale = TimeScale.MONTHS;
  
  // 20 =20%
  uint256 public interestRate = 20;
  
  // 40=40% out of intersetRate
  uint256 public interestRateToTribeOne = 40;
  ...
  event LoanPayment(
    ...
    uint256 interestToTribeOnePerInstallement, 
    Status status
  );
  ...
  /**
   *  Construct a new lending contract
   * @param _nftAddress The address of the TribeOne nft collection
   */
  ...
  function createLoan(
    ...
    // Transfer the items from lender to tribeOne contract
    _transferItems(
        msg.sender, 
        address(this), 
        nftAddressArray, 
  ...
  function payLoan(uint256 loanId) external payable {
    ...
    uint256 discount = calculateDiscount(msg.sender);
    uint256 interestToTribeOnePerInstallement = interestPerInstallement.mul(interestRateToTribeOne).div(100);

    if ( discount != 1 ){
        if ( loans[loanId].currency == address(0) ){
            require(msg.sender.send(interestToTribeOnePerInstallement.div(discount)), "Discount returnation failed");
            amountPaidAsInstallmentToLender = amountPaidAsInstallmentToLender.sub(interestToTribeOnePerInstallement.div(discount));
        }
        interestToTribeOnePerInstallement = interestToTribeOnePerInstallement.sub(interestToTribeOnePerInstallement.div(discount));
    }
    amountPaidAsInstallmentToLender = amountPaidAsInstallmentToLender.sub(interestToTribeOnePerInstallement);
    ...
    _transferTokens(msg.sender,loans[loanId].lender,loans[loanId].currency,amountPaidAsInstallmentToLender,interestToTribeOnePerInstallement);
    ...
    emit LoanPayment(
      ...
      interestToTribeOnePerInstallement,
      loans[loanId].status
    );
  }
  ...
  /**
   * @notice Liquidity mining participants or TribeOne NFT holders will be able to get some discount
   * @param requester The address of the requester
   */
  function calculateDiscount(address requester) public view returns(uint256){
    for (uint i = 0; i < TribeOneNftTokenIdArray.length; ++i)
	    if ( IERC1155(nftAddress).balanceOf(requester,TribeOneNftTokenIdArray[i]) > 0 )
  ...
  function getLoanInstallmentCost(
    ...
    interestDiscounted = interestPerInstallement.mul(interestRateToTribeOne).div(100).div(discount); // amount of interest saved per installment
    interestToTribeOnePerInstallement = interestPerInstallement.mul(interestRateToTribeOne).div(100).sub(interestDiscounted);
    amountPaidAsInstallmentToLender = interestPerInstallement.mul(uint256(100).sub(interestRateToTribeOne)).div(100); 
  }
   ...
   * @param _discountNft Discount value for the TribeOne NFT holders
   * @param _discountGeyser Discount value for the TribeOne liquidity mining participants
   * @param _geyserAddressArray List of the TribeOne Geyser contracts 
   * @param _TribeOneNftTokenIdArray Array of TribeOne nft token IDs.
   * @param _nftAddress List of the TribeOne NFT collections
   */
  function setDiscounts(uint32 _discountNft, uint32 _discountGeyser, address[] calldata _geyserAddressArray, uint256[] calldata _TribeOneNftTokenIdArray, address _nftAddress) external onlyOwner {
    discountNft = _discountNft;
    discountGeyser = _discountGeyser;
    geyserAddressArray = _geyserAddressArray;
    TribeOneNftTokenIdArray = _TribeOneNftTokenIdArray;
    nftAddress = _nftAddress;
  }
  
  /**
   * @notice Setter function
   * @param _promissoryNoteContractAddress The address of the TribeOne promissory Note contract
   ...
   * @param _interestRateToTribeOne Value of interest rate to TribeOne
   * @param _lenderFee Value of the lender fee
   */
  function setGlobalVariables(address _promissoryNoteContractAddress, uint256 _ltv, uint256 _installmentFrequency, TimeScale _installmentTimeScale, uint256 _interestRate, uint256 _interestRateToTribeOne, uint32 _lenderFee) external onlyOwner {
    ltv = _ltv;
    installmentFrequency = _installmentFrequency;
    installmentTimeScale = _installmentTimeScale;
    interestRate = _interestRate;
    interestRateToTribeOne = _interestRateToTribeOne;
  ...
  function addNftTokenId(uint256 nftId) external onlyOwner {
      TribeOneNftTokenIdArray.push(nftId);
  }
  ...
  /**
   * @notice Transfer eth or erc20 tokens from an account to another
   * @param from From account address.
   * @param to To account address.
  ...
  function _transferTokens(
      ...
      if ( currency != address(0) ){
          ...
          require(IERC20(currency).transferFrom(
              ...
              owner(), 
              qty2
          ), "Transfer of tokens to TribeOne failed");
      }else{
          require(to.send(qty1), "Transfer of ETH to receiver failed");
          require(payable(owner()).send(qty2), "Transfer of ETH to TribeOne failed");
      }
  }
